...
/*  f  */ void f102() { u = st.b[p++]; if (u == '+') { st.f[s - 1] += st.f[s]; s--; }
            ...
            else if (u == 'O') { y=INT2POINTER(st.i[s--]); TOS = POINTER2INT(fopen(y, TOS ? "wb" : "rb")); }
            else if (u == 'C') { if (TOS) { fclose(INT2FILEPTR(TOS)); } s--; }
            else if (u == 'R') { t = TOS; TOS = 0; if (t) fread((void*)&TOS, 1, 1, INT2FILEPTR(t)); }
            else if (u == 'W') { fwrite((void*)&NOS, 1, 1, INT2FILEPTR(TOS)); s -= 2; } }
/*  i  */ void f105() { u=st.b[p++]; st.i[u+ro]++; }
/*  l  */ void f108() { u=st.b[p++]; if (u=='@') { TOS=POINTER2INT(TOS); } /*else if (u=='!') { *((int *)TOS)=NOS; s-=2; }*/ }
/*  m  */ void f109() { u=st.b[p++]; if (u=='@') { TOS=POINTER2INT(TOS); } /*else if (u=='!') { *((char*)TOS)=NOS; s-=2; }*/ }
...
int main(int argc, char *argv[]) {
    ...
    for (int i=1; i<argc; ++i) { y=argv[i]; t=atoi(y); st.i['0'+ro+i] = t?t:POINTER2INT(y); }
    if ((argc > 1) && (argv[1][0]!='-')) {
